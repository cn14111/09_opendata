...
Opensource data increases the number of predictors from the data 
...
field
```
stronger the fiber, resist the breakage
...
how many rows in the dataset--> nrow()
unique sites --> distinct(site)

What is the statistical distribution of fiber strength? (we can use geon_density, histogram, box plot)

--> geom_density is used to know the statistical distribution
```{r fiber strength}
ggplot(data = field) +
...
```
filter(state_abbr %in% c("PR", "AK", "HI")) #this will keep those states
"%in%" used to filter more than one state (if one state need to be filtered then we use "==")

1)to get the boundaries of the US states
states is the geospatial object in R and we are gonna make a map using that.
...
Undo the pivot_longer, use the pivot_wider
...
unnest all of the cells not a single cell
no.of site years times no of days in a year, we have those many rows (A tibble:254,770 × 19)
